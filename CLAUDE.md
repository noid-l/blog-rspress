...
基于 Rspress 的博客（React 版本）。文章直接在 `docs/posts/` 维护，文章数据完全由 Rspress 官方运行时 API（`usePages()`）驱动，无任何生成脚本。
...
├── posts/                  # 博客文章（直接在此维护，命名 YYYY-MM-DD-slug.md）
│   └── index.mdx           # 文章归档页（pageType: custom → PostsLayout）
├── tags.mdx                # 标签页（pageType: custom → TagsLayout）
...
文章直接在 `docs/posts/` 中维护，frontmatter 字段：`title` / `date` / `tags` / `description` / 可选 `category` / 可选 `draft: true`（草稿不出现在列表和 RSS）。新增文章无需任何额外步骤，dev/build 自动生效。
